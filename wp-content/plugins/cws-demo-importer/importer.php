...
define('CWS_DEMO_URL', 'http://up.creaws.com');
...
function splitXml($file, $options) {
	$features = array();
	...
	if (empty($features) || in_array('content', $features) || in_array('all', $features)) {
		...
		while(2===2) {
			...
			if ($item_e - $item > 30000 || $should_break) {
	...
	echo '<div id="cws_imp_err" style="display:none">Something went wrong. Most likely due to operation timeout. Please, contact us providing your url, login and password so we could help you sort it out.</div>';
...
class WP_CWS_Demo_Import extends WP_Importer {
	...
	function greet() {
		...
		if ($f >= 1) {
			$woo_text .= '<p>Please note: WooCommerce plugin is not installed, therefore the WooCommerce dummy data will not be installed.</p>Press this button to begin import.';
		} else {
...
<?php
	...
	function unpack_package($delete_package = true) {
		...
		$demo_url = 'http://up.creaws.com/demo/' . $package;
