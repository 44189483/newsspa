...
/*
*/
if ( !shortcode_exists( 'cws-grid' ) ) {
	function shortcode_cws_grid ($args, $content){
		$atts_default = array(
			'atts' => '{"_cols":"1"}',
		);
		...
		$grid_class = '';
		if (has_filter('cwsfe_grid_class')) {
			$grid_class = apply_filters('cwsfe_grid_class', '');
		}
		$row_atts = '';
		if (has_filter('cwsfe_row_atts')) {
			$row_atts = apply_filters('cwsfe_row_atts', $atts); // !!!
		}
		$out = '<div class="'. $grid_class . '"'. $row_atts .'>';
		$out .= do_shortcode($content);
		$out .= "</div>";
		return $out;
	}
	add_shortcode('cws-grid','shortcode_cws_grid');
}
...
if ( !shortcode_exists( 'cws-widget' ) ) {
	...
	if (!$pbf_redeclares || !in_array( PB_THEME_SLUG.'_cws_pricecol_renderer', $pbf_redeclares) ) {
...
if (!$pbf_redeclares || !in_array( PB_THEME_SLUG.'_cws_text_renderer', $pbf_redeclares) ) {
...
if (!$pbf_redeclares || !in_array( PB_THEME_SLUG.'_cws_tabs_renderer', $pbf_redeclares) ) {
...
if ( !shortcode_exists( 'item' ) ) {
	if (!$pbf_redeclares || !in_array(PB_THEME_SLUG.'_cws_item_shortcode', $pbf_redeclares) ) {
		function cws_item_shortcode0 ($atts, $content) {
			extract( shortcode_atts(
				array(
						'type' => NULL,
						'open' => NULL,
						'title' => __('TITLE',PB_THEME_SLUG),
						'iconfa' => '',
						'iconimg' => ''
				), $atts));
...
function cws_pb_ourteam_shortcode($args) {
	...
	$pb_options = function_exists(PB_THEME_SLUG.'_get_pb_options') ? call_user_func(PB_THEME_SLUG.'_get_pb_options') : null;
	...
	if ($r->have_posts()) {
		while ( $r->have_posts() ) {
			...
			$output .= '<img src="' . cws_thumb($img[0], array('width' => 48, 'height' => 48) ) . '" alt="">';
			$output .= '</div></div>';
		}
	...
	if ($r->have_posts()) {
		while ( $r->have_posts() ) {
			...
			if ($thumb_id) {
				$output .= '<div class="pic" data-id="' . $thumb_id . '">';
				$img = wp_get_attachment_image_src( $thumb_id, 'thumbnail' );
				$output .= '<img src="' . cws_thumb($img[0], array('width' => 64, 'height' => 64) ) . '" alt="">';
				$output .= '</div>';
			}
...
function cws_pb_tweet_shortcode($args) {
	...
	$twt_obj = call_user_func(PB_THEME_SLUG . '_getTweets', '', $items, false );
	if ($twt_obj && is_array($twt_obj)) {
		if (!array_key_exists('error', $twt_obj)) {
			...
			if ($twt_obj) {
				...
				$output .= '<li>' . __( 'Twitter API keys and tokens are not set.', PB_THEME_SLUG ) . '</li>';
			}
			$output .= '</ul>';
		} else {
...
if (!$pbf_redeclares || !in_array(PB_THEME_SLUG.'_cws_tab_item_handler', $pbf_redeclares) ) {
...
if (!$pbf_redeclares || !in_array(PB_THEME_SLUG.'_cws_accordion_renderer', $pbf_redeclares) ) {
...
if (!$pbf_redeclares || !in_array(PB_THEME_SLUG.'_cws_callout_renderer', $pbf_redeclares) ) {
	function cws_callout_renderer ($args) {
		extract(shortcode_atts(
			array(
				...
				'iconimg' => '',
				'title' => '',
				'c_btn_text' => __('Click Me', PB_THEME_SLUG),
				'c_btn_href' => '#',
			), $args['atts']));
...
if (!$pbf_redeclares || !in_array(PB_THEME_SLUG.'_cws_accordion_item_renderer', $pbf_redeclares) ) {
	function cws_accordion_item_renderer ( $title, $content, $open, $iconfa, $iconimg ){
		...
		$out .= "<div class='accordion_title'>" . ( $featured ? ( !empty( $iconfa ) ? "<i class='acc_featured_icon fa fa-$iconfa'></i>" : ( !empty( $iconimg ) ? "<span class='acc_featured_img'><img width='$iconimg_size' height='$iconimg_size' src='" . ( cws_thumb( $iconimg, array( 'width' => $iconimg_size . 'px', 'height' => $iconimg_size . 'px' ) ) ) . "' /></span>" : "" ) ) : "" ) . "$title<i class='accordion_icon'></i></div>"; // TITLE
