	...
	jQuery(form).find('ul.groups').each(function(a, el){
		jQuery(el).find('li').each(function(b, el) {
			...
			jQuery(el).find('[name]').each(function(c, elm) {
				elm.name = elm.name.replace(/(.+\[)(\d+)(\].*)/, '$1'+that+'$3');
			});
		});
	});
}
...
jQuery(document).ready(function() {
	...
	// tabs
	jQuery('div[id^=cws-post-metabox-id-] .inside .cws_pb_ftabs a').on('click', function(e) {
	...
	jQuery('div[id^=cws-post-metabox-id-] .inside input[name="cws_mb_is_allday"]').on('change', function(e) {
		tp.periodpicker('setOption', 'timepicker', !e.target.checked);
	});

	jQuery('div[id^=cws-post-metabox-id-] .inside input[data-default-color],.widget-liquid-right .widget-content input[data-default-color]').each(function(){
	...
	jQuery('div[id^=cws-post-metabox-id-] .inside .recurring_events li .close').each(function(e, k) {
		addCloseProcessing(k);
	});

	var mb_prefix = 'cws_mb_';

	jQuery('div[id^=cws-post-metabox-id-] .inside .recurring_events>div>button').on('click', function() {
			...
			if ( group.hasOwnProperty(gkey) ) {
				...
				for (var item in group[gkey]) {
					if (group[gkey].hasOwnProperty(item)) {
						...
						var input = jQuery(parent).find('input[name="' + group_prefix +'"],select[name="' + group_prefix +'"],textarea[name="' + group_prefix +'"]')[0];
						// value = group[gkey]
						switch (input.type) {
							case 'text':
							case 'number':
	...
	function getWidgetId (el) {
		var id = jQuery(el).closest('.row').length ? jQuery(el).closest('.row').parent().data('w') : jQuery(el).find('.row').parent().data('w');
		return (undefined !== id) ? id : 0;
	}

	jQuery('div[id^=cws-post-metabox-id-] .inside').each(function(e, k){
	...
	function initGroupFunctions(parent) {
		...
		jQuery(parent).find('textarea[data-templ="group_template"]').each(function(){
	...
	jQuery(document).ajaxSuccess(function(e, xhr, settings) {
		if (undefined !== settings.data && 'string' === typeof settings.data) {
